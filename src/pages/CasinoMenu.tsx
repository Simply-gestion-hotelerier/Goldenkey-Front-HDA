// src/pages/casino/CasinoMenu.tsx
...
export default function CasinoMenu() {
  ...
  const loadDishes = async () => {
    try {
      ...
      toast({ title: "Erreur chargement articles casino", variant: "destructive" });
    }
  };
  ...
  const onSubmit = async (values: z.infer<typeof barDishFormSchema>) => {
    ...
    try {
      ...
      if (editingDish) {
        await api.patch(`/casino/dishes/${editingDish.id}`, payload);
        toast({ title: "Article casino mis à jour" });
      } else {
        await api.post("/casino/dishes", payload);
        toast({ title: "Article casino créé" });
      }
  ...
  const deleteDish = async (id: number) => {
    ...
    try {
      ...
      toast({ title: "Article casino supprimé" });
      await loadDishes();
    } catch (error: any) {
        ...
        <main className="flex-1 overflow-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-2">
                <Dices className="h-8 w-8 text-yellow-600" /> Carte du Casino
              </h1>
            ...
            return (
              <div key={key}>
                <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
                  <span className="h-1 w-6 bg-yellow-500 rounded-full inline-block" />
                  {label}
          ...
          {filteredDishes.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm">Créez votre premier article du casino</p>
            </div>
          )}
