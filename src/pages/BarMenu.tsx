...
export default function BarMenu() {
  ...
  const loadDishes = async () => {
    try {
      const response = await api.get<any>("/bar/dishes");
      const data = response?.data ?? response ?? [];
      setDishes(Array.isArray(data) ? data : []);
    } catch (error) {
      toast({ title: "Erreur chargement articles bar", variant: "destructive" });
    }
  };

  const loadItems = async () => {
    try {
      const response = await api.get<any>("/bar/dishes/for-bar");
      const data = response?.data ?? response ?? [];
      ...
      setIngredients((prev) =>
        prev.map((i) =>
          i.itemId === selectedItem.id
            ? { ...i, quantity: newQty, cost: newQty * unitCost }
            : i
        )
      );
    } else {
  ...
  const onSubmit = async (values: z.infer<typeof barDishFormSchema>) => {
    ...
    try {
      ...
      if (editingDish) {
        await api.patch(`/bar/dishes/${editingDish.id}`, payload);
        toast({ title: "Article bar mis à jour" });
      } else {
        await api.post("/bar/dishes", payload);
        toast({ title: "Article bar créé" });
      }
  ...
  const deleteDish = async (id: number) => {
    ...
    try {
      ...
      toast({ title: "Article bar supprimé" });
      await loadDishes();
    } catch (error: any) {
  ...
  const toggleActive = async (dish: BarDish) => {
    try {
      await api.patch(`/bar/dishes/${dish.id}`, { isActive: !dish.isActive, ingredients: dish.ingredients });
      await loadDishes();
  ...
  const handleExport = async (format: string) => {
    ...
    try {
      ...
      if (format === "excel") {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Carte Bar");
        ...
        const lines = ["=== CARTE DU BAR ===", "", ...exportData.map((d) => `[${d.categorie}] ${d.nom} — ${fmt(d.prix)} Ar`)];
  ...
  // ── Derived state for quantity validation display ────────────────────────────
    ...
    <div className="flex h-screen bg-background">
        ...
        <main className="flex-1 overflow-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-2">
                <Wine className="h-8 w-8 text-purple-600" /> Carte du Bar
              </h1>
            ...
            return (
              <div key={key}>
                <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
                  <span className="h-1 w-6 bg-purple-500 rounded-full inline-block" />
                  {label}
          ...
          {filteredDishes.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm">Créez votre premier article de bar</p>
            </div>
          )}
      ...
      <Dialog open={showDishDialog} onOpenChange={setShowDishDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingDish ? "Modifier l'article" : "Nouvel article bar"}</DialogTitle>
            <DialogDescription>
              {editingDish ? `Modifier "${editingDish.name}"` : "Créer un nouvel article pour la carte du bar"}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showIngredientDialog} onOpenChange={(open) => !open && closeIngredientDialog()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Sélectionner un article du stock bar</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  // Allow digits and a single decimal point only; ignore other characters silently
                  if (raw === "" || /^\d*\.?\d*$/.test(raw)) {
